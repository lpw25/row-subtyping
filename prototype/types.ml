...
module Printing = struct
  ...
  let rec print_type_desc ctx raw aliased names nongen desc =
    ...
    let aliased =
      ...
      | Ref t ->
          let aliased = print_type In_ref raw aliased names nongen t in
          printf " ref";
          aliased
      | Unit ->
          printf "unit";
  ...
  and print_kind raw aliased names nongen =
    ...
    | Range(t1, t2) ->
        ...
        | Bot _, _ ->
            printf "[- ";
            let aliased = print_type In_range raw aliased names nongen t2 in
            printf "]";
            aliased
        | _, Top _ ->
            printf "[+ ";
            let aliased = print_type In_range raw aliased names nongen t1 in
            printf "]";
            aliased
        | _, _ ->
            printf "[+ ";
